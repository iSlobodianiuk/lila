  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <button
        ...
        className={[
          "relative h-24 w-24 rounded-2xl border border-white/60 bg-gradient-to-br from-white to-stone-50",
      ...
      <button
        ...
        onClick={onRoll}
        disabled={disabled || isRolling}
        className="rounded-full bg-stone-900/90 px-5 py-2 text-sm font-medium tracking-wide text-stone-50 shadow-md transition hover:bg-stone-900 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {isRolling ? "Кидаємо…" : label}
      </button>
      {hint && (
        <p className="max-w-[14rem] text-center text-xs leading-snug text-stone-500">
          {hint}
        </p>
      )}
    </div>
